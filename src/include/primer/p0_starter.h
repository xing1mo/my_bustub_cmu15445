...
class RowMatrix : public Matrix<T> {
 public:
  ...
  T GetElement(int i, int j) const override {
    if (i >= this->rows_ || i < 0) {
      throw Exception(ExceptionType::OUT_OF_RANGE, "row out");
    }
    if (j >= this->cols_ || j < 0) {
      throw Exception(ExceptionType::OUT_OF_RANGE, "col out");
    }
    return data_[i][j];
  }
  ...
  void SetElement(int i, int j, T val) override {
    ...
    this->linear_[i * this->rows_ + j] = data_[i][j] = val;
  }
  ...
  void FillFrom(const std::vector<T> &source) override {
    ...
    if (static_cast<int>(source.size()) != this->rows_ * this->cols_) {
      throw Exception(ExceptionType::OUT_OF_RANGE, "fillSize err");
    }
    for (int i = 0; i < static_cast<int>(source.size()); ++i) {
      this->linear_[i] = data_[i / this->cols_][i % this->cols_] = source[i];
    }
  }
...
class RowMatrixOperations {
 public:
  ...
  static std::unique_ptr<RowMatrix<T>> Multiply(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB) {
    ...
    for (int i = 0; i < matrixA->GetRowCount(); ++i) {
      for (int j = 0; j < matrixB->GetColumnCount(); ++j) {
        T x = matrixA->GetElement(i, 0) * matrixB->GetElement(0, j);
        for (int k = 1; k < matrixA->GetColumnCount(); ++k) {
          x = x + matrixA->GetElement(i, k) * matrixB->GetElement(k, j);
        }
  ...
  static std::unique_ptr<RowMatrix<T>> GEMM(const RowMatrix<T> *matrixA, const RowMatrix<T> *matrixB,
    ...
    if (tmp == nullptr) {
      auto result = std::make_unique<RowMatrix<T>>(matrixC->GetRowCount(), matrixC->GetColumnCount());
      for (int i = 0; i < matrixC->GetRowCount(); ++i) {
        for (int j = 0; j < matrixC->GetColumnCount(); ++j) {
          result->SetElement(i, j, matrixC->GetElement(i, j));
        }
      }
      return result;
    }
